...
scoreboard players remove @e[scores={sonic_blast_cooldown=1..20}] sonic_blast_cooldown 1
execute as @a[scores={sonic_blast_cooldown=0}] run title @s actionbar [{"text":" "}]
scoreboard players reset @e[scores={sonic_blast_cooldown=0}] sonic_blast_cooldown
execute as @a[scores={sonic_blast_cooldown=1..20}] run title @s actionbar [{"translate":"enchantments.tcc.sonic_blast.cooldown.prefix","fallback":"还有"},{"score":{"objective":"sonic_blast_cooldown","name":"@s"}},{"translate":"enchantments.tcc.sonic_blast.cooldown.suffix","fallback":"刻冷却完毕"}]
